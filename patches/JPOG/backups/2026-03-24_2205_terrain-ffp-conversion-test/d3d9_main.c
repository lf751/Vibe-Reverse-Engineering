...
/* Runtime A/B toggles loaded from proxy.ini */
int g_cfgWorldOverrideOnDP = 1;
int g_cfgWorldOverrideOnDIP = 1;
int g_cfgForceIdentityView = 0;
int g_cfgRejectIdentityView = 1;
int g_cfgPreserveWorldOnMvStall = 1;
int g_cfgTerrainQ2LatchGuard = 1;
int g_cfgTerrainShaderBypass = 1;
...
__declspec(dllexport) void* __stdcall Direct3DCreate9(unsigned int SDKVersion) {
    ...
    if (!g_realD3D9) {
        ...
        g_cfgWorldOverrideOnDP = GetPrivateProfileIntA(
            "ABTest", "WorldOverrideOnDP", 1, iniBuf);
        g_cfgWorldOverrideOnDIP = GetPrivateProfileIntA(
            "ABTest", "WorldOverrideOnDIP", 1, iniBuf);
        g_cfgForceIdentityView = GetPrivateProfileIntA(
            "ABTest", "ForceIdentityView", 0, iniBuf);
        g_cfgRejectIdentityView = GetPrivateProfileIntA(
            "ABTest", "RejectIdentityView", 1, iniBuf);
        g_cfgPreserveWorldOnMvStall = GetPrivateProfileIntA(
            "ABTest", "PreserveWorldOnMvStall", 1, iniBuf);
        g_cfgTerrainQ2LatchGuard = GetPrivateProfileIntA(
            "ABTest", "TerrainQ2LatchGuard", 1, iniBuf);
        g_cfgTerrainShaderBypass = GetPrivateProfileIntA(
            "ABTest", "TerrainShaderBypass", 1, iniBuf);

        log_int("Config WorldOverrideOnDP=", g_cfgWorldOverrideOnDP);
        log_int("Config WorldOverrideOnDIP=", g_cfgWorldOverrideOnDIP);
        log_int("Config ForceIdentityView=", g_cfgForceIdentityView);
        log_int("Config RejectIdentityView=", g_cfgRejectIdentityView);
        log_int("Config PreserveWorldOnMvStall=", g_cfgPreserveWorldOnMvStall);
        log_int("Config TerrainQ2LatchGuard=", g_cfgTerrainQ2LatchGuard);
        log_int("Config TerrainShaderBypass=", g_cfgTerrainShaderBypass);
