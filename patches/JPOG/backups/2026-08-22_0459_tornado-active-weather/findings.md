...
- Controller weather fields are `previousWeather` at `+0x4C`, `currentWeather` at `+0x54`, and `nextWeather` at `+0x58`.
...
| 1 | `lightrain` | `drizzle` |
| 2 | `heavyrain` | `rainstorm` |
| 3 | `tornado` | `thunderstorm` |
| 4 | `heatwave` | `sandstorm` |

The ASI initializes the bridge export `remixapi_InitializeLibrary`, obtains `remixapi_Interface.SetGameValue`, and sends `__weather.blend_seconds=11.0` before `__weather.target=<preset>`. Duplicate weather IDs are suppressed.
