"""Patch d3d9_device.c: replace draw_terrain_with_world_dip with enhanced version."""
import sys
    ...
    unsigned int si, unsigned int pc)
{
    ...
    if (!g_terrainFFPFired) {
        ...
        /* Query actual D3D9 state VIEW and PROJ (what Remix will see) */
        ((FN_GetTransform)vt[45])(self->pReal, D3DTS_VIEW, deviceView);
        ((FN_GetTransform)vt[45])(self->pReal, D3DTS_PROJECTION, deviceProj);
        ...
        /* Decompose PROJ: if PROJ = V*P combined, set real P = inv(V) * VP */
...
with open('d3d9_device.c', 'r') as f:
    content = f.read()

# Find the start and end of draw_terrain_with_world_dip
...
new_content = content[:idx_start] + NEW_DIP + content[idx_end:]

with open('d3d9_device.c', 'w') as f:
    f.write(new_content)

print("Patched draw_terrain_with_world_dip successfully")
print(f"Old section: {idx_end - idx_start} chars")
print(f"New section: {len(NEW_DIP)} chars")
